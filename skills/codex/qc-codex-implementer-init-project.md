---
name: qc-codex-implementer-init-project
...
# Skill: qc-codex-implementer-init-project
...
3. Decide business-area tags: parse `--areas` flag or infer from `requirements.md` / task brief. If tags cannot be inferred safely, STOP with `needs_input: areas` and list exact candidate areas; do not call Claude-only tools such as `AskUserQuestion`.
